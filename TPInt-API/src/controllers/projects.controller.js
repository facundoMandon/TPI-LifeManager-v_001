...
export const getProjects = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error interno del servidor al obtener proyectos." });
  } finally {
...
export const getProjectbyId = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error interno del servidor al obtener el proyecto." });
  } finally {
...
export const createProject = async (req, res) => {
  try {
    ...
    if (!name || !initDate || !state) {
      console.warn(
        "Faltan campos obligatorios (name, initDate, state) al crear proyecto."
      );
      return res
        .status(400)
        .json({
          message: "Nombre, fecha de inicio y estado son obligatorios.",
        });
    }
    ...
    res
      .status(500)
      .json({ message: "Error interno del servidor al crear el proyecto." });
  } finally {
...
export const updateProject = async (req, res) => {
  try {
    ...
    if (!projectToUpdate) {
      console.warn(
        `Proyecto con ID ${projectId} no encontrado para actualización.`
      );
      return res.status(404).json({ message: "Proyecto no encontrado." });
    }
    ...
    if (
      projectToUpdate.userId !== authenticatedUserId &&
      authenticatedUserRole !== "admin" &&
      authenticatedUserRole !== "superadmin"
    ) {
      console.warn(
        `Usuario ${authenticatedUserId} (rol: ${authenticatedUserRole}) intentó actualizar el proyecto ${projectId} (dueño: ${projectToUpdate.userId}) sin permisos.`
      );
      return res
        .status(403)
        .json({ message: "No tienes permiso para actualizar este proyecto." });
    }
    ...
    if (updatedRowsCount === 0) {
      console.warn(
        `El proyecto con ID ${projectId} no pudo ser actualizado (0 filas afectadas).`
      );
      return res
        .status(200)
        .json({ message: "Proyecto actualizado sin cambios o no encontrado." });
    }
...
export const deleteProject = async (req, res) => {
  try {
    ...
    if (!projectToDelete) {
      console.warn(
        `Proyecto con ID ${projectId} no encontrado para eliminación.`
      );
      return res.status(404).json({ message: "Proyecto no encontrado." });
    }
    ...
    if (
      authenticatedUserRole !== "superadmin" &&
      projectToDelete.userId !== authenticatedUserId
    ) {
      console.warn(
        `Usuario ${authenticatedUserId} (rol: ${authenticatedUserRole}) intentó eliminar el proyecto ${projectId} (dueño: ${projectToDelete.userId}) sin permisos de superadmin o propiedad.`
      );
      return res
        .status(403)
    ...
    if (deletedRowsCount === 0) {
      console.warn(
        `El proyecto con ID ${projectId} no pudo ser eliminado (0 filas afectadas).`
      );
      return res
        .status(404)
        .json({ message: "Proyecto no encontrado o ya eliminado." });
    }
    ...
    res
      .status(500)
      .json({ message: "Error interno del servidor al eliminar el proyecto." });
  } finally {
...
export const getProjectsByUserId = async (req, res) => {
  try {
    ...
    console.log("Usuario autenticado (req.user):", req.user);
    console.log(
      "ID de usuario solicitado (req.params.userId):",
      req.params.userId
    );
    ...
    if (
      ...
      console.warn(
        `Usuario ${authenticatedUserId} (rol: ${authenticatedUserRole}) intentó acceder a proyectos del usuario ${targetUserId} sin permisos.`
      );
      return res
        .status(403)
        .json({ message: "No tienes permiso para ver estos proyectos." });
    }
    ...
    console.log(
      `Proyectos encontrados para el usuario ${targetUserId}: ${projects.length}`
    );
    res.status(200).json(projects);
  } catch (error) {
